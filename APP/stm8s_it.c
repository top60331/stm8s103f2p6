...
/**
 ...
 INTERRUPT_HANDLER(TIM4_UPD_OVF_IRQHandler, 23)
 {
  /* In order to detect unexpected events during development,
     it is recommended to set a breakpoint on the following instruction.
  */
   if(TIM4_GetITStatus(TIM4_IT_UPDATE) ==  SET) //判断是否是定时器4产生了溢出中断
   {
     TIM4_ClearITPendingBit(TIM4_IT_UPDATE);    //如果是产生了溢出中断，那么要清除中断标志位
     
     cnt_1s++;
     
     if (cnt_1s > 1000) {
       STATUS_LED_TOG(); // add CJH  
       cnt_1s = 0;
       F_1sec = TRUE;
     }
